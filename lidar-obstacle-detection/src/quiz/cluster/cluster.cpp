...
void proximity(int point, const std::vector<std::vector<float>>& points, std::vector<int>& cluster, std::unordered_set<int>& processed, KdTree* tree, float distanceTol) {
	if (processed.find(point) == processed.end()) {
		...
		for (int n_point : nearby) {
			proximity(n_point, points, cluster, processed, tree, distanceTol);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for (int pt = 0; pt < points.size(); pt++) {
		if (processed.find(pt) == processed.end()) {
			// point has not been processed

			// create new cluster
			std::vector<int> cluster;
			proximity(pt, points, cluster, processed, tree, distanceTol);
